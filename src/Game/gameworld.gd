...
func createEntity(name: String, grid_pos: Vector2i, chest_contents = null):
	...
	match name:
		"Ammo":
			new_entity = Entity.new(grid_pos, entity_types.ammo, map_data)
			new_entity.position.y -= 16
		"Knife":
			new_entity = Entity.new(grid_pos, entity_types.knife, map_data)
			new_entity.position.y -= 16
		"Lucky Charm":
	...
	SignalBus.entity_created.emit(grid_pos, player.grid_position)
...
func _physics_process(_delta: float) -> void:
	if !combat_manager.has_begun:
		...
		if action:
			...
			var item_in_room = get_map_data().get_item_at_location(player.grid_position)
			gun_bob = true
			bob_timer = 0
